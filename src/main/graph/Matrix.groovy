...
class Matrix {
	...
	def Matrix power(int n, Matrix m)  {
		if (n < 0)
  ...
  // Chronecker multiply
  def Matrix chronecker(N = new Matrix(getRef())) {
    def M = new Matrix(getRef())

    def result
    (1..M.getRowsNumber()).each { row ->
      def rowResult = N * M[row][1]
      (2..M.getColumnsNumber()).each { col ->
        N * M[row][col] >>> rowResult
      }
      if(!result)
        result = rowResult
      else
  ...
  def static Matrix chronecker(M, n) {
    def result = M
    (2..n).each {
      result = result.chronecker(M)
    }
    result
  }
